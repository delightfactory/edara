...
import { getPurchaseOrders, getPurchaseOrder, createPurchaseReturn } from '@/lib/services/purchases'
import type { PurchaseOrderWithRefs, PurchaseReturnInput } from '@/lib/types/purchases'
...
export function PurchaseReturnFormPage() {
    usePageTitle('إنشاء مرتجع مشتريات')
    ...
    useEffect(() => {
        getPurchaseOrders({ pageSize: 200 }).then(r => setOrders(r.data.filter(o => o.status !== 'draft' && o.status !== 'cancelled'))).catch(() => {})
    }, [])
    ...
    const handleSubmit = async () => {
        if (!orderId || !order) { toast.error('يرجى اختيار أمر الشراء'); return }
        if (selectedItems.length === 0) { toast.error('يرجى اختيار بند واحد على الأقل'); return }
        setSaving(true)
        const userId: string = profile?.id || ''
        try {
            const returnInput: PurchaseReturnInput = {
                supplier_id: order!.supplier_id, warehouse_id: order!.warehouse_id,
                purchase_order_id: orderId, branch_id: order!.branch_id ?? null, return_date: returnDate,
                total_amount: totalAmount, reason: reason || null, notes: notes || null,
                items: selectedItems.map(i => ({
                    product_id: i.product_id, unit_id: i.unit_id, quantity: i.quantity, unit_price: i.unit_price,
                    total: i.quantity * i.unit_price, conversion_factor: i.conversion_factor, base_quantity: i.quantity * i.conversion_factor,
                })),
            }
            await createPurchaseReturn(returnInput, userId)
            toast.success('تم إنشاء مرتجع المشتريات')
            navigate('/purchases/returns')
        } catch (err: unknown) { toast.error(err instanceof Error ? err.message : 'فشل الإنشاء') }
    ...
    return (
        <div className="space-y-6 animate-[fade-in_0.4s_ease-out]">
            <div className="flex items-center gap-3">
                <button onClick={() => navigate('/purchases/returns')} className="btn btn-ghost"><ArrowRight className="h-5 w-5" /></button>
                ...
                <div>
                    <h1 className="page-title">مرتجع مشتريات جديد</h1>
                    <p className="page-subtitle">اختر أمر الشراء والبنود المراد إرجاعها</p>
                </div>
            </div>
            ...
            {order && (
                <div className="edara-card p-4">
                    <h3 className="text-sm font-bold mb-2">ملخص أمر الشراء</h3>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                        <div><span style={{ color: 'var(--text-muted)' }}>المورد: </span><strong>{order.supplier?.name}</strong></div>
                        ...
                        <div><span style={{ color: 'var(--text-muted)' }}>المخزن: </span><strong style={{ color: 'var(--text-secondary)' }}>{order.warehouse?.name || '—'}</strong></div>
            ...
            <div className="flex justify-end gap-3">
                <button onClick={() => navigate('/purchases/returns')} className="btn btn-secondary">إلغاء</button>
